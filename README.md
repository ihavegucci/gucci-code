...
Всё остальное подчинено дешевизне: 5 файлов, 4 этапа, ~62 000 знаков инструкций за прогон,
...
Скопировать **всю папку** `gucci-code` в каталог скиллов Claude Code.

**Для всех проектов** (личный скилл):

```bash
cp -r gucci-code ~/.claude/skills/
```

**Только для текущего проекта** (командный скилл, уезжает в репозиторий вместе с кодом):

```bash
mkdir -p .claude/skills && cp -r gucci-code .claude/skills/
```

Копируется папка целиком — `SKILL.md` без `phases/` и `dashboard.html` работать не будет:
...
└── README.md          этот файл — не нужен скиллу для работы, только людям
```
...
**Чего здесь нет:** режима с согласованием каждого шага и отдельного круга вылизывания.
И то и другое — паузы, а скилл делался ради их устранения. Не понравился результат — скажи,
что именно не так, и это будет новый прогон: так дешевле и честнее, чем бесконечный круг
доводки.
...
Больше 10 — честно скажет, что задача крупная, и **всё равно соберёт**.
...
На тестовых прогонах он поймал: отсутствующую выгрузку в PDF; что тег сделали необязательным,
хотя в задаче было «должен быть тег»; и что поиск не ищет по тегам. Ни одну из трёх не
заметили ни планировщик, ни исполнители.
...
## Когда не стоит его звать

- **Ты хочешь писать код вместе, строка за строкой.** Тогда просто работай с Claude напрямую —
  скилл заточен на то, чтобы отдать результат, а не на совместное редактирование.
- **Правка в две строки.** Накладные расходы на разбор задачи и приёмку больше самой правки.
- **Проект крупнее 10 кусков.** Соберёт, но честно скажет, что задача великовата: с такой
  лучше идти двумя прогонами, а не одним.
- **Нужно согласовывать каждый шаг.** Этого режима здесь нет намеренно.

---

## Что проверено, а что нет

Скилл прогнан от начала до конца **трижды, на живых задачах**, с настоящими субагентами и
работающим кодом:

- **T0, полный автомат** — CLI отчёта по расходам из CSV. 9 тестов, слепая приёмка нашла
  реальный дефект.
- **T2, полуавтомат** — CLI заметок на SQLite. Вопрос пользователю, 5 кусков, 3 волны,
  два исполнителя параллельно, `BLOCKED`, блокировка зависимого куска, 19 тестов.
- **T0, полуавтомат/глубоко** — редизайн собственного дашборда скилла (тема, лого). Слепая
  приёмка нашла частичное совпадение с референсом — часть визуала потребовала бы правки
  `<script>`, а это было прямо запрещено брифом; записано в отчёт как есть, не подчищено.

Найдено и починено больше двадцати ошибок, каждая починка проверена запуском.

**Не проверялось ни разу** — если споткнётся, скорее всего здесь:

1. Счётчик ремонтов «две неудачи и стоп» — вживую не срабатывал.
2. Режим «допроси» в живой сборке — гонялись только автомат и полуавтомат.
3. Возобновление после сжатия контекста.
4. Глубина «строго» — не пробовалась. «Глубоко» прогнана один раз, на T0 (редизайн
   дашборда): держит те же гейты, углубляет требования через `R##.n`, ведёт себя как описано.
5. Живое обновление дашборда раз в 10 секунд — проверялось только на подставленных данных,
   не на реальном тикающем прогоне.

Первые два — самые вероятные источники проблем. Первый настоящий прогон лучше запустить на
чём-то, что не жалко.
...
**Дашборд не открылся** — не ошибка, прогон идёт дальше. Открой
`.gucci/dashboard.html` вручную.

**Дашборд пустой или замер** — сломался `state.js`. Проверить:
`tail -n +2 .gucci/state.js | python -m json.tool`.

**Кусок помечен сломанным** — так и задумано: скилл не долбится в стену, а идёт дальше и
пишет об этом в отчёте. Смотри «Что пошло не по плану».
